...
export class WebsocketLimitedPriceProvider implements IPriceProvider {
  private _logger!: any;
  private _usdtUsdProvider!: IPriceProvider;
  private readonly _factor!: any;
  private readonly _type!: any;
  private readonly _pair!: string;
  private readonly _exchanges!: any;
  private readonly _name2client!: any;
  private readonly _ex2priceInfo!: any;
  private readonly _forceUsdtUsdConversion!: boolean;
  ...
  init(): void {
    for (const { ex, market, client } of this._exchanges) {
  ...
  async getUsdtConversionRate(market: string): Promise<number> {
    if (market.endsWith('USDT') && !!this._usdtUsdProvider) {
      const rate: number = (await this._usdtUsdProvider.getPrice()) || 1.0;
  ...
  marketId(clientStr: string, base: string, quote: string): string {
    if (
      ...
      (clientStr == 'kraken' || clientStr == 'bitmex') &&
      base == 'BTC'
    ) {
      ...
      if (
        ['ADA', 'XRP', 'XLM', 'BTC', 'ETH', 'FIL', 'LTC', 'SGB'].indexOf(
          base,
        ) >= 0 &&
        quote == 'USDT'
      ) {
  ...
  subscribe(ex: string, pair: string): void {
    const tmp: any = pair.split('/');
    const base: string = tmp[0];
    const quote: string = tmp[1];
    const marketObj = {
      id: this.marketId(ex, base, quote),
      base,
      quote,
      type: 'spot',
    };
  ...
  async getRestPrice(): Promise<number> {
    const prices = [];
    for (const { ex, market } of this._exchanges) {
      try {
        const ccxtex = new (ccxt as any)[ex]({ timeout: 2 * 1000 });
        const ticker = await ccxtex.fetchTicker(market);
        if (ticker) {
  ...
  async getPrice(): Promise<number> {
    ...
    if (this._exchanges.length > 0) {
      const conversionRate: number = await this.getUsdtConversionRate(
        this._exchanges[0].market,
      );
      for (const { ex } of this._exchanges) {
        const priceInfo = this._ex2priceInfo[ex];
        // price shall not be older than 3min
        if (
          priceInfo &&
          priceInfo.price &&
          priceInfo.priceTime > Date.now() - 1000 * 60 * 3
        ) {
    ...
    if (prices.length > 0) {
      const price = this.getAveragePrice(prices);
  ...
  getAveragePrice(prices: any): number {
    if (prices.length == 0) {
      ...
      return (
        (prices.reduce((a: any, b: any) => a + b, 0.0) / prices.length) *
        this._factor
      );
    }
  }
